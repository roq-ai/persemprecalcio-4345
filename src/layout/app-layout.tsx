...
import { getSportsApps } from 'apiSdk/sports-apps';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'editor', 'contributor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SportsAppInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/sports-apps` : null),
    () => getSportsApps({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Sports App',
      path: isTenantUser ? `/sports-apps/view/${data?.[0]?.id}` : '/sports-apps',
      entity: 'sports_app',
